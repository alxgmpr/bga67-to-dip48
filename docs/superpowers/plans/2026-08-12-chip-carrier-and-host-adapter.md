...
5. **Open Pcbnew and run Tools > Update PCB from Schematic (F8), with "Replace footprints with those specified in the schematic" ticked.** Do **not** substitute the `sync_schematic_to_board` MCP tool for this. It was tried on 2026-08-13: it adds new footprints and assigns nets, but it does not replace changed footprints, and it rewrites every pad and track net to un-prefixed names (`IO6`, `{slash}CE`) while leaving the original `/`-prefixed declarations in place, splitting each net in two. The board had to be restored with `git checkout chip/chip.kicad_pcb`. `chip/chip.kicad_pcb` was copied from the carrier, so until this runs it still carries the *mirrored* BGA footprint and the DF40 *plug*. `tools/build_chip.py` in Task 5 flips whatever footprints it finds; if it runs first it will flip the wrong ones and Task 6's `"Mirrored" not in name` assertion will fail. `U1` and `J1` keep their positions through the update; `C1` and `C2` arrive unplaced and get positioned in Task 5 Step 3.
